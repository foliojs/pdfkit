 ...
 * @typedef {Object} Position<T>
 * @property {T} [x]
 * The x coordinate
 *
 * Defaults to the current document position `doc.x`
 * @property {T} [y]
 * The y coordinate
 *
 * Defaults to the current document position `doc.y`
 */
 ...
 *
 * @template T
 * @typedef {T | T[] | { vertical: T, horizontal: T } | ExpandedSideDefinition<T>} SideDefinition<T>
 **/
...
export function definedProps(obj) {
  return Object.fromEntries(
    Object.entries(obj)
      .map(([k, v]) => {
        if (v !== undefined && v !== null && typeof v === "object" && !Array.isArray(v)) return [k, definedProps(v)];
        return [k, v];
      })
...
export function deepClone(a) {
  if (a === undefined || a === null || typeof a !== "object") return a;
