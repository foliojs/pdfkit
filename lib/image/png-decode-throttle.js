...
// Use an event queue coroutine to hold back decoding and avoid running out of
// memory.
//
// There are two producer-consumer lists:
//   - Decode list: The first keeps the low-memory pending to decode and starts
//     decoding as soon as there is available memory. When decoded it adds it to
//     the process list.
...
export function pngDecodePixelsThrottle(pngImagePdfElement, pixelsCallback) {
  ...
  //  ---NOTE: Here #1 is not freed due to a node bug https://github.com/nodejs/node/issues/45303 ---
