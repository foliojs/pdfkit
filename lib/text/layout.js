...
export default class LayoutEngine {
  ...
  withFont(font, callback) {
    ...
    const res = callback();
    if (font.src || font.size) {
      this.doc.font(rollbackFont, rollbackFontFamily, rollbackFontSize);
    }
    return res;
  }
  ...
  boundsOfString(text, options) {
    ...
    return this.withFont(textOptions.font, () => {
      ...
      for (const [, width, height] of this.#constrainedLines(
        text,
        maxWidth,
        maxHeight,
        textOptions,
      ...
      const [x, y, width, height] = rotateBox(
        textOptions.x,
        textOptions.y,
        contentWidth,
        contentHeight,
  ...
  textBox(text, options) {
    ...
    if (
      textBoxOpts.rotation ||
      (textBoxOpts.border &&
        textBoxOpts.borderColor &&
        textBoxOpts.borderOpacity)
    ) {
      ...
      this.box(textBoxOpts.x, textBoxOpts.y, bounds.width, bounds.height, {
        ...textBoxOpts,
        rotation: 0,
      });
  ...
  box(x, y, width, height, options) {
    const {
      ...
      backgroundColor,
      backgroundOpacity,
    } = this.#normalizeBoxOptions(options);
    ...
    if (backgroundColor !== 'transparent' && backgroundOpacity > 0) {
      this.doc
        .save()
        .fillColor(backgroundColor, backgroundOpacity)
    ...
    if (
      [bco.right, bco.bottom, bco.left].every((side) => {
        side.every((s, i) => s === bco.top[i]);
      })
    ) {
  ...
  #normalizeTextOptions(options) {
    ...
    this.withFont(opts.font, () => {
      opts.x = this.doc.sizeToPoint(options.x, this.doc.x);
      opts.y = this.doc.sizeToPoint(options.y, this.doc.x);
      opts.width = this.doc.sizeToPoint(
        options.width,
        this.doc.page.width - this.doc.page.margins.right - opts.x,
      );
      opts.height = this.doc.sizeToPoint(
        options.height,
        this.doc.page.height - this.doc.page.margins.bottom - opts.y,
      );

      opts.rotation = (options.rotation ?? 0) % 360;
      if (opts.rotation < 0) opts.rotation += 360;

      opts.horizontalScaling = options.horizontalScaling ?? 100;
      opts.characterSpacing = options.characterSpacing ?? 0;
      opts.features = options.features ?? [];

      opts.ellipsis = options.ellipsis ?? false;
      if (typeof opts.ellipsis === 'boolean') {
  ...
  #normalizeBoxOptions(options, defaultBorder = 1) {
    options = options ?? {};
    const opts = {};
    opts.border = normalizeSides(options.border, defaultBorder);
    opts.borderColor = normalizeSides(options.borderColor, 'black');
    opts.borderOpacity = normalizeSides(options.borderOpacity, 1);
    opts.rotation = normalizeRotation(options.rotation, 0);
    opts.backgroundColor = options.backgroundColor ?? 'transparent';
    opts.backgroundOpacity = options.backgroundOpacity ?? 1;
  ...
  #normalizeTextBoxOptions(options) {
    options = options ?? {};
    const textOpts = this.#normalizeTextOptions(options);
    const boxOpts = this.#normalizeBoxOptions(options, 0);

    const opts = {};
    Object.assign(opts, textOpts, boxOpts);

    opts.alignContent = normalizeAlignment(options.alignContent, {
      x: 'left',
      y: 'top',
    });
  ...
  #widthOfSegment(text, options) {
    ...
    const fontSize = this.doc._font.widthOfString(
      text,
      this.doc._fontSize,
      options.features,
  ...
  *#lines(text, options) {
    ...
    for (const [line, width, height] of this.#constrainedLines(
      text,
      maxWidth,
      maxHeight,
      options,
  ...
  *#constrainedLines(text, maxWidth, maxHeight, options) {
    ...
    let currLine,
      currLineWidth = 0;
    for (const [newLine, newLineWidth] of this.#widthConstrainedLines(
      text,
      maxWidth,
      options,
    )) {
      ...
      if (currLine) {
        ...
        if (currHeight + lineGap + lineHeight > maxHeight) {
          let truncLine = '';
          let truncLineWidth = this.#widthOfSegment(options.ellipsis, options);

          // Quick check make sure the ellipsis can fit, if not return a blank string
          if (truncLineWidth > maxWidth) {
            // No new lines can fit so terminate early
            return undefined;
          }

          // Incrementally build the truncated line per grapheme
          for (const { segment: grapheme } of SEGMENTER.segment(
            currLine.trimEnd(),
          )) {
            const newTruncLine = truncLine + grapheme;
            // Unfortunately we cant use any width caching here due to kerning pairs
            const newTruncLineWidth = this.#widthOfSegment(
              newTruncLine + options.ellipsis,
              options,
            );

            // The new grapheme is too big for the line
            if (newTruncLineWidth > maxWidth) break;

            truncLine = newTruncLine;
            truncLineWidth = newTruncLineWidth;
          }

          // No gap as it's the last line, the width already includes the ellipsis
          yield [truncLine + options.ellipsis, truncLineWidth, lineHeight];
