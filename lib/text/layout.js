...
export default class LayoutEngine {
  ...
  constructor(doc, options) {
    /** @type {any} */
    this.doc = doc;
    this.options = this.#normalizeLayoutOptions(options);
  }
  ...
  box(x, y, width, height, options) {
    const { border, borderColor, borderOpacity, rotation } = this.#normalizeBoxOptions(options);
   ...
   */
  #normalizeLayoutOptions(options) {
    options = options ?? {};
  ...
  #widthOfSegment(text, options) {
    ...
    // Ideally we could extract the number of graphemes/glyphs directly in the font
    const numGraphemes = Array.from(SEGMENTER.segment(text)).length;
    const fontSize = this.doc._font.widthOfString(text, this.doc._fontSize, options.features);
    const charSpacing = (numGraphemes - 1) * options.characterSpacing;
    return (fontSize + charSpacing) * options.horizontalScaling / 100;
  }
  ...
  * #constrainedLines(text, maxWidth, maxHeight, options) {
    ...
    let currLine;
    for (let newLine of this.#widthConstrainedLines(text, maxWidth, options)) {
      ...
      const newLineWidth = this.#widthOfSegment(newLine, options);
      ...
      if (currLine) {
        ...
        if (currHeight + lineGap + lineHeight > maxHeight) {
          ...
          for (const { segment: grapheme } of SEGMENTER.segment(currLine.line.trimEnd())) {
