...
class PDFGradient {
  ...
  stop(pos, color, opacity) {
    ...
    if (this.stops.length === 0) {
      if (this._activeColorSpace) {
        // always prefer _activeColorSpace
        this._colorSpace = this._activeColorSpace;
      } else {
        // fallback
        if (color.length === 3) {
    ...
    // validate
    if (this._colorSpace === this._activeColorSpace) {
      const profile = this._colorProfiles[this._colorSpace];
