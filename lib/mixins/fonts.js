...
import { segmentEmojiText } from '../emoji/segmenter';
...
export default {
  initFonts(
    ...
    // Emoji font state
    this._emojiFont = null; // fontkit font object for emoji
    this._emojiImageCache = new Map(); // glyphId+ppem → PDFImage object
  ...
  /**
   * Register an emoji font for color emoji rendering.
   * The font is loaded via fontkit and stored on the document.
   * Emoji glyphs from this font are rendered as image XObjects (SBIX)
   * rather than embedded CIDFont text.
   *
   * @param {string|Uint8Array|ArrayBuffer} src - Path or buffer of the emoji font
   * @param {string} [family] - Font name within a TTC collection (e.g. 'AppleColorEmoji')
   * @returns {this}
   */
  registerEmojiFont(src, family) {
    this._emojiFont = PDFFontFactory.openEmoji(src, family);
    return this;
  },

  /**
   * Segment text into plain-text and emoji runs.
   * Returns null if no emoji font is registered (fast path).
   *
   * @param {string} text
   * @returns {Array<{type: 'text'|'emoji', text: string}>|null}
   */
  _segmentEmojiText(text) {
    if (!this._emojiFont) return null;
    const segments = segmentEmojiText(text);
    // If there's only one text segment covering the whole string, no emoji detected
    if (
      segments.length === 1 &&
    ...
    // If there are any emoji segments, return the segmentation
    if (segments.some((s) => s.type === 'emoji')) {
      return segments;
    }
    return null;
  },
  ...
  /**
   * Calculate the width of an emoji string using the emoji font metrics.
   * Uses font shaping (layout) to get the advance width of the full sequence,
   * which handles ZWJ sequences, regional indicators, etc.
   *
   * @param {string} text - The emoji text (a single emoji sequence)
   * @param {number} fontSize - The font size in points
   * @returns {number} Width in points
   */
  _emojiWidthOfString(text, fontSize) {
    if (!this._emojiFont) return 0;
    try {
      const run = this._emojiFont.layout(text);
      let totalAdvance = 0;
      for (let i = 0; i < run.glyphs.length; i++) {
        totalAdvance += run.positions[i].xAdvance;
      }
      return (totalAdvance / this._emojiFont.unitsPerEm) * fontSize;
    } catch {
      // Some fonts (e.g. NotoColorEmoji CBDT) don't support layout() because
      // fontkit cannot create glyph objects (no glyf/CFF table).
      // Fall back to fontSize per emoji — emoji are typically square.
      return fontSize;
    }
  },
