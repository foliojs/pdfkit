 ...
 */
export function computeInscribedBounds(
  boundingWidth,
  boundingHeight,
  rotation,
) {
  ...
  if (rotation === 90 || rotation === 270) {
    return [boundingHeight, boundingWidth];
  }
  ...
  if (EF + FG > boundingHeight) {
    ...
    if (rotation < 90 || (rotation > 180 && rotation < 270)) {
      textMaxWidth = (boundingWidth * cos - boundingHeight * sin) / den;
      textMaxHeight = (boundingHeight * cos - boundingWidth * sin) / den;
    } else {
      textMaxHeight = (boundingWidth * cos - boundingHeight * sin) / den;
      textMaxWidth = (boundingHeight * cos - boundingWidth * sin) / den;
    }
  }
