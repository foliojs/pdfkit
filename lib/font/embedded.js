...
class EmbeddedFont extends PDFFont {
  ...
  layoutRun(text, features) {
    ...
    run.numGlyphs = run.glyphs.length;
  ...
  layout(text, features, onlyWidth) {
    ...
    while (index <= text.length) {
      ...
      if (
        ...
        [' ', '\t'].includes((needle = text.charAt(index)))
      ) {
        ...
        // Keep track of the number of glyphs
    ...
    return { glyphs, positions, advanceWidth, numGlyphs };
  }
  ...
  widthOfString(string, size, features, glyphSpacing) {
    const layout = this.layout(string, features, true);
    const width = layout.advanceWidth;
    const scale = size / 1000;
    return width * scale + (glyphSpacing ?? 0) * ((layout.numGlyphs || 1) - 1);
  }
