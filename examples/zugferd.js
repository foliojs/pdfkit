...
const doc = new PDFDocument({
  ...
  info: {
    ...
    Subject: 'ZUGFeRD XRechnung e-invoice',
    Keywords: 'invoice zugferd xrechnung',
  },
});

doc.pipe(fs.createWriteStream(path.join(__dirname, 'zugferd.pdf')));
...
doc.einvoice('zugferd', path.join(__dirname, 'einvoices', 'zugferd-xrechnung.xml'), { profile: 'xrechnung' });
...
doc.fillColor('#AAAAAA').fontSize(6.5)
  .text('ZUGFeRD / XRechnung - PDF/A-3b', LEFT, PAGE_BOTTOM - 12, { width: WIDTH, align: 'right' });
